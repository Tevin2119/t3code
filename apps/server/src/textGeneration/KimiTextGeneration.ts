 ...
 * branch names, thread titles) through a throwaway `kimi acp` session, the same
 * way Grok does. Reusing ACP rather than `kimi -p` keeps one code path for
 * spawning the CLI and gives structured streaming instead of the banner-and-
 * bullets text mode.
 *
 * @module textGeneration/KimiTextGeneration
 */
  ...
  const runKimiJson = <S extends Schema.Top>({
    ...
    Effect.gen(function* () {
      ...
      const promptResult = yield* Effect.gen(function* () {
        ...
        const currentModel = currentKimiModelIdFromSessionSetup(started.sessionSetupResult);
        if (requestedModel) {
          const resolved = resolveKimiAcpModelId(requestedModel);
