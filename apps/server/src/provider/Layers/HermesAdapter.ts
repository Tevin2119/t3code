 ...
 *
 * Keeps one `hermes acp` process per thread and translates its ACP traffic into
 ...
 * lives here is only the Hermes-specific lifecycle.
 *
 * Shaped like the Kimi adapter rather than the Antigravity and Grok ones:
 * Hermes' ACP surface has no subagent batches and no backgrounded commands, so
 * there is no task linkage to track. Conversation rewind is absent from the
 * agent too, and `rollbackThread` reports that rather than faking it.
 *
 * Hermes does advertise session modes (`default`, `accept_edits`) alongside the
 * plan updates this forwards. The modes stay untouched: every tool call still
 * arrives as `session/request_permission`, so T3 Code's own approval flow
 * covers the same ground without a second policy to keep in sync.
 *
 * Hermes picks its own inference provider and model, so a turn only sends a
 * model id when the user chose one — see `resolveHermesAcpModelId`.
 *
 ...
 * offered. ACP option `kind`s are standard, so no Hermes-specific mapping is
 ...
 * never shows a choice Hermes would reject. Cancel is always available because
 * the client can abandon a request regardless of the offered options.
 */
export function hermesApprovalOptions(
  request: NativePermission,
...
export interface HermesAdapterOptions {
  ...
  readonly makeRuntime: (
    input: Omit<HermesAcpRuntimeInput, "childProcessSpawner" | "hermesSettings" | "environment">,
...
export const makeHermesAdapter = Effect.fn("makeHermesAdapter")(function* (
    ...
    Effect.mapError(
      (cause) =>
        new ProviderAdapterRequestError({
          ...
          detail: "Could not create a Hermes event ID.",
          cause,
        }),
    ),
  );
  ...
  const cancelRequests = Effect.fn("HermesAdapter.cancelRequests")(function* (
  ...
  const handlePermission = Effect.fn("HermesAdapter.handlePermission")(function* (
    context: SessionContext,
    request: NativePermission,
    ...
    const permissionRequest = {
      ...parsed,
      detail: parsed.detail ?? "Hermes requests permission.",
    };
    return yield* Effect.gen(function* () {
      yield* emit(
        makeAcpRequestOpenedEvent({
          ...
          permissionRequest,
          approvalOptions: hermesApprovalOptions(request),
  ...
  const handleEvent = Effect.fn("HermesAdapter.handleEvent")(function* (
    context: SessionContext,
      ...
      Effect.gen(function* () {
        if (!settings.enabled) {
          return yield* new ProviderAdapterValidationError({
            ...
            issue: "Enable Hermes in provider settings before starting a thread.",
          });
        }
        if (
        ...
        if (input.resumeCursor !== undefined && Option.isNone(cursor)) {
          return yield* new ProviderAdapterValidationError({
            provider: PROVIDER,
            operation: "startSession",
            issue: "The saved Hermes session is invalid. Start a new thread.",
          });
        }
        ...
        return yield* Effect.gen(function* () {
              ...
              ? handlePermission(context, request).pipe(
                  Effect.mapError((cause) =>
                    EffectAcpErrors.AcpRequestError.internalError(
                      "Could not process a Hermes permission request.",
                      undefined,
                      { cause },
                    ),
                  ),
          ...
          const currentModel = currentHermesModelIdFromSessionSetup(started.sessionSetupResult);
          const requestedModel = resolveHermesAcpModelId(input.modelSelection?.model);
          const model = requestedModel ?? currentModel;
          ...
          yield* Stream.runForEach(runtime.getEvents(), (event) =>
            ...
            Effect.catchCause(() => Effect.logError("Could not process a Hermes runtime event.")),
  ...
  const sendTurn: Adapter["sendTurn"] = Effect.fn("HermesAdapter.sendTurn")(function* (input) {
    ...
    if (!text) {
      return yield* new ProviderAdapterValidationError({
        provider: PROVIDER,
        operation: "sendTurn",
        issue: "Hermes turns need a prompt.",
      });
    }
    ...
    return yield* Effect.gen(function* () {
      const launch = yield* context.promptLock.withPermit(
        Effect.gen(function* () {
          ...
          const selectedModel = resolveHermesAcpModelId(input.modelSelection?.model);
          if (selectedModel) {
            yield* context.runtime.setSessionModel(selectedModel);
          }
          context.session = {
            ...context.session,
            status: "running",
            ...
            .prompt(
              {
                prompt: [
                  ...
                  {
                    type: "text",
                    text: buildRuntimeInstructions({ harness: "Hermes", model }),
                  },
                ],
              },
              { dispatched },
  ...
  const respondToRequest: Adapter["respondToRequest"] = (threadId, requestId, decision) =>
    Effect.gen(function* () {
      ...
      const optionId =
        decision === "cancel"
          ? undefined
          : selectHermesPermissionOptionId(pending.request, decision);
      if (decision !== "cancel" && optionId === undefined) {
        return yield* new ProviderAdapterValidationError({
          provider: PROVIDER,
          operation: "respondToRequest",
          issue:
            "Hermes did not offer this permission choice. Select one of the available choices.",
        });
      }
      yield* Deferred.succeed(pending.response, {
        decision,
  ...
  const respondToUserInput: Adapter["respondToUserInput"] = (threadId, _requestId, _answers) =>
    Effect.gen(function* () {
      ...
      // `hermes acp` advertises no elicitation capability, so nothing ever opens a
      // structured question for the client to answer.
      return yield* new ProviderAdapterValidationError({
        provider: PROVIDER,
        operation: "respondToUserInput",
        issue: "Hermes does not ask structured questions.",
      });
    });
    ...
    stopAll().pipe(
      Effect.catchCause((cause) =>
        Cause.hasInterrupts(cause)
          ? Effect.void
          : Effect.logError("Could not stop a Hermes session."),
      ),
    ...
    rollbackThread: (_threadId: ThreadId, _numTurns: number) =>
      Effect.fail(
        new ProviderAdapterValidationError({
          ...
          issue: "Hermes does not support conversation rewind. Start a new thread instead.",
        }),
      ),
